...
(async() => {
    while (true){
        try{
            console.log(1)
            let button = document.querySelector(".dw-comment-more_comment")
            button.click();
            await sleep(1)
        }catch(e){
            console.log(e)
...
setTimeout(function(){
    ...
    try{
        var tt = document.querySelector(".publish-time").innerText
        TemplateData.created_at = new Date(tt.substr(0, tt.length -1)).getTime();
        TemplateData.title = document.querySelector(".title").innerText;
        TemplateData.url = window.location.href;
        TemplateData.article = BeeUtils.htmlToJson(document.getElementsByClassName("show-content")[0]);
        var comment_list = document.querySelectorAll(".comment-wrap p");
        for(var c of comment_list){
