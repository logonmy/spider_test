        ...
        function recursiveFormat(htmlnode) {
            ...
            for (var i = 0; i < length; i++) {
                ...
                if (isNewLineTag(node.nodeName.toLowerCase())) {
                    ...
                    var img = {};
                    img.img = image;
                    if (img.src && image.src.indexOf("http") != 0) {
                        continue;
                    }
...
setTimeout(function(){
    try{
        ...
        if(window.location.href.indexOf("365yg") > -1){
            ...
            //头条自己的文章
            var count = 0
            while(document.querySelector("[ga_event=click_loadmore_comment]") && count < 20){
                document.querySelector("[ga_event=click_loadmore_comment]").click();
                count++;
            }
            ...
            for(let i=0;i<comments.length;i++){
                TemplateData.comments.push(comments[i].querySelector("p").innerText);
            }
