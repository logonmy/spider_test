...
try{
    TemplateData.title = document.querySelector("h1.title").textContent;
    TemplateData.article = BeeUtils.htmlToJson(document.getElementsByClassName("article-holder")[0]);
    TemplateData.url = window.location.href;
    TemplateData.category = document.querySelectorAll(".category-link span")[0].textContent;
    var tags = document.querySelectorAll(".tag-container li span");

    for(var i=0;i < tags.length;i++){
        TemplateData.tags.push(tags[i].textContent);
    }

    var readCount = document.querySelectorAll(".article-data span")[0].textContent;
    TemplateData.readCount = parseInt(readCount.substr(0, readCount.length - 2));
    var likeCount = document.querySelectorAll(".article-data span")[1].textContent;
    TemplateData.likeCount = parseInt(likeCount.substr(0, likeCount.length - 2));

    if(document.querySelector(".banner-img-holder")){
        TemplateData.cover_img.src = document.querySelector(".banner-img-holder").getAttribute("style").split('"')[1].split("@")[0];
        TemplateData.cover_img.height = document.querySelector(".banner-img-holder").clientHeight;
        TemplateData.cover_img.width = document.querySelector(".banner-img-holder").clientWidth;
    }

    var urlLength = window.location.href.split("/").length;
    var no = window.location.href.split("/")[urlLength-1].substring(2);
    var baseUrl = "https://api.bilibili.com/x/v2/reply?&jsonp=jsonp&pn=1&type=12&oid=" + no;
    liteAjax(baseUrl, function(d){
        d = JSON.parse(d);
        console.log(d)
        var length = d.data.replies.length;
        for(var i = 0;i < length;i++){
            TemplateData.comments.push(d.data.replies[i].content.message)
        }
        var created_at = document.getElementsByClassName("create-time")[0].textContent;
        if(created_at.split("-").length< 3){
