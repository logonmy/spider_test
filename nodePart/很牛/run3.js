...
// let part = "";
// readLine("bhsbHtml.txt").go(async(data, next) => {
//     if(data.indexOf("==这是一个完美的分隔符==") < 0){
//         part = part + data;
//         next();
//     }else{

//         let html = "<div id='hhsshh'>" + part + "</div>";
//         let d = new jsdom.JSDOM(html);
//         d = d.window.document.querySelector("#hhsshh");
//         let s = BeeUtils.htmlToJson(d)
//         console.log(s);
//         //File.appendFileSync("finally.txt", JSON.stringify(s) + "\n") 
...
File.readFile('henniu.txt', 'utf8', function(err, data){
    console.log(data);  
    data = data.split("==这是一个完美的分隔符==")
    for(let da of data){
        let html = "<div id='hhsshh'>" + da + "</div>";
        let d = new jsdom.JSDOM(html);
        d = d.window.document.querySelector("#hhsshh");
        let s = BeeUtils.htmlToJson(d)
        console.log(s);
        File.appendFileSync("other.txt", JSON.stringify(s) + "\n") 
    }
});