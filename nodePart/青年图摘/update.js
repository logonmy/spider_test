...
let getNewestHref = async () => {
    let url = "https://qingniantuzhai.com/";
    let html = await getPage(url);
    let d = new jsdom.JSDOM(html);
    let document = d.window.document;
    url = "https://qingniantuzhai.com/" + document.querySelector(".content a").getAttribute("href");
    html = await getPage(url);
    d = new jsdom.JSDOM(html);
    document = d.window.document;
    let a = BeeUtils.htmlToJson(document.querySelector(".kg-card-markdown"));
    let result = []
